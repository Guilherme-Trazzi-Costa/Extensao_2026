...
# Chave de assinatura do cookie, gerada uma vez quando o módulo é
# carregado — sobrevive a quedas de conexão, mas não a reinícios do
# container (a menos que "cookie_secret" esteja no secrets.toml).
_SECRET_FALLBACK = _rnd.token_hex(32)
...
def _secret_key() -> str:
    ...
    sem ela, usa a chave gerada ao carregar o módulo (some no reinício)."""
...
def _tentar_login_por_cookie() -> bool:
    ...
    token = _cookie_manager().get(_COOKIE_NOME)
    if not token:
        return False
...
def exigir_login() -> dict:
    ...
    # Sem sessão em memória (ex: WebSocket reconectou do zero) —
    # tenta restaurar pelo cookie antes de exigir senha de novo.
    if _tentar_login_por_cookie():
        return st.session_state[_USUARIO]
